...
public class ShapeDragMediator implements DragMediator {
    ...
    public Point convertToWhiteboardCoordinates(CustomPanel panel, Point panelPoint) {
        ...
        try {
            ...
            // Check if the point is within the whiteboard bounds
            if (screenPoint.x >= whiteboardLocation.x &&
                screenPoint.x < whiteboardLocation.x + whiteBoard.getWidth() &&
                screenPoint.y >= whiteboardLocation.y &&
                screenPoint.y < whiteboardLocation.y + whiteBoard.getHeight()) {
